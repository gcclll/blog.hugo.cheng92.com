...
参数： ~#+begin_solution "question?" :reprimand "answer?" :really "what?"~

分别是 green:最外层标题(question), black:第二层标题(answer), red:最内层标题(what)

#+begin_solution "question?" :reprimand "answer?" :really "what?"
The answer is 2.
...
~$BLOG/src/css/block.scss~ 中增加各种 block 的样式。

通过 ~#+begin_xxx-block ... #+end_xxx-block~ 增加对应的 ~xxx~ block

然后通过 snippets 快捷创建 block，如：

~1iw=warn-block~, ~1is=success-block~, ... 等等。
...
@include gen-block('question', #f3f5f7, #000000, #42b983);
